...
local function close_redis(red)  
    ...
    if not ok then  
        waf_log.error_log("set keepalive error : "..err)  
    end  
end
...
local function signcheck(params,secret)
	--判断参数是否为空，为空报异常
	if waf_log.isTableEmpty(params) then
		local mess="params table is empty"
        waf_log.error_log(mess)
        return false,mess
	end
...
SignAuthHandler.version = "0.1.0"

function SignAuthHandler:new()
    SignAuthHandler.super.new(self, "sign_auth-plugin", self.version)
    waf_log.debug_log("===========SignAuthHandler.new============");
end

function SignAuthHandler:access()
    SignAuthHandler.super.access(self)
	waf_log.debug_log("===========SignAuthHandler.access============");
	local params = {}
	...
	if not ok then  
		close_redis(red)
		waf_log.error_log("Cannot connect");
