...
local function close_redis(red)  
    ...
    if not ok then  
        waf_log.error_log("redis set keepalive error : "..err)  
    end  
end
...
LimitIpHandler.version = "0.1.0"

function LimitIpHandler:new()
    LimitIpHandler.super.new(self, "limit_ip-plugin", self.version)
    waf_log.debug_log("===========LimitIpHandler.new============");
end

function LimitIpHandler:access()
    LimitIpHandler.super.access(self)
	waf_log.debug_log("===========LimitIpHandler.access============");
	...
	if not ok then  
		waf_log.error_log("limit ip cannot connect redis");
	else
		local ip_blacklist, err = red:smembers(key);
		if err then
			waf_log.error_log("limit ip smembers");
		else
			--刷新本地缓存，重新设置
